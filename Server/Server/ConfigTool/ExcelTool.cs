...
public class ExcelTool {
	...
	{
        FileStream stream = File.Open(@"..\..\Xlsx\" + filename + ".xlsx", FileMode.Open, FileAccess.Read);
	...
	public static bool HandleATable(DataTable result){
        ...
        SaveTableData(datalist, result.TableName);
        return true;
	}
    ...
    public static void SaveTableData(List<ConfigClass> datalist ,string tablename){
        ...
        WriteByteToFile(dicdata, tablename);
    }
    ...
    public static void WriteByteToFile(byte[] data, string name)
    {
        //string pathEnd = path.Split('/')[path.Split('/').Length-1];
        //string folderPath = path.Remove(path.Length - (pathEnd.Length + 1)); //+1代表斜杠
        string folderPath = @"..\..\ConfigTable\";
        if (!Directory.Exists(folderPath))
            Directory.CreateDirectory(folderPath);
        FileStream fs = new FileStream(folderPath + name, FileMode.Create);
