...
namespace LuaFramework
{
    ...
    {
        string m_BaseDownloadingURL = "";  // 获取资源的地方，加载AssetBundle包的时候会用到
        string[] m_AllManifest = null;
        AssetBundleManifest m_AssetBundleManifest = null;  //包间依赖关系文件，从这个类中的信息中可以知道某个包依赖的包有哪些，如果依赖的包还没加载进去则先加载依赖包
        Dictionary<string, string[]> m_Dependencies = new Dictionary<string, string[]>();  //如上，一个包名对应多个依赖包名
        Dictionary<string, AssetBundleInfo> m_LoadedAssetBundles = new Dictionary<string, AssetBundleInfo>();  //已经加载入内存的AssetBundle包，一个包名对应一个AssetBundle（AssetBundleInfo包了一层）
        Dictionary<string, List<LoadAssetRequest>> m_LoadRequests = new Dictionary<string, List<LoadAssetRequest>>();  //当前正在处理的“从AssetBundle包中解压Asset资源（纹理，材质，预设等）”的请求，一个包名对应多个Asset资源名
        ...
        public void LoadAssetSprite(string abName, string[] assetNames, LuaFunction action = null)
        {
            LoadAsset<Sprite>(abName, assetNames, null, action);
        }
        ...
        IEnumerator OnLoadAsset<T>(string abName) where T : UObject
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                if (list[i].luaFunc != null)
                {
                    ...
                    T obj = (T)result.ToArray()[0];
                    list[i].luaFunc.Call(obj);
