...
function Game.InitEnums()
    for i = 1, #EnumNames do 
        require ("Enum/"..tostring(EnumNames[i]))
    end
end

function Game.InitPanels()
...
function Game.InitTools()
    for i = 1, #ToolNames do
        require ("Tools/"..tostring(ToolNames[i]))
    end
end

function Game.InitCtrls()
    for key,value in pairs(CtrlNames) do
        require ("Controller/"..tostring(value))
        loadstring("return "..value)():Awake();
    end
end
...
function Game.OnInitOK()
    ...
    UpdateBeat:Add(function ()
        UIMgr:Trigger('UpdateHeadPosition')
    end)
	...
	local ctrl = CtrlManager.GetCtrl(CtrlNames.LoginBg);
	if ctrl ~= nil then
		ctrl:Awake();
	end
    --]]
    PanelMgr:OpenPanel('LoginBgPanel',UILayer.Bottom,MaskEnum.None)
    PanelMgr:OpenPanel('LoginPanel',UILayer.Middle,MaskEnum.None)
