namespace Net.Server
{
    ...
    public class TcpServerIOCP<Player,Scene> : TcpServerBase<Player, Scene> 
        where Player : NetPlayer, new() 
        where Scene : NetScene<Player>, new()
    {
        //客户端连接池
        private List<SocketAsyncEventArgs> ioContextPool = new List<SocketAsyncEventArgs>();
        ...
        public override void Start(int port = 666, int workerThreads = 0)
        {
            ...
            for (int i = 0; i < LineUp; i++) {
                ...
                args.SetBuffer(new byte[50000], 0, 50000);
                ioContextPool.Add(args);
            }
            AcceptConnect(null);
        ...
        //开始接受客户端连接
        private void AcceptConnect(SocketAsyncEventArgs args)
        {
            try
            {
                if (!IsRunServer)
        ...
        private SocketAsyncEventArgs Pop()//与栈相反
        {
            if (stack >= ioContextPool.Count)
                return null;
            var args = ioContextPool[stack];
            stack++;
            return args;
        }
            ...
            if (client.Connected)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        AddUnClient(client);
                        if (!client.ReceiveAsync(args))//如果已经接收到数据,那么就要进入处理数据
                            ReceiveBuffer(args);
                    }
                }
        ...
        void OnIOCompleted(object sender, SocketAsyncEventArgs args)
        {
        ...
        //异步接受数据缓冲区
        void ReceiveBuffer(SocketAsyncEventArgs args)
        {
            Socket client = (Socket)args.UserToken;
            // 检查远程主机是否关闭连接
            if (args.BytesTransferred > 0) {
                try {
                    var buffer = new byte[args.BytesTransferred];//获取接收到的数据
                    Array.Copy(args.Buffer, 0, buffer, 0, buffer.Length);
                    int count = args.BytesTransferred;
                    TcpHandle(buffer, count, client);
                } catch {
                }
                try {
        ...
        protected override void Send(Player client, byte[] buffer, int index, int count)
        {
            try
            {
                ...
                using (SocketAsyncEventArgs args = new SocketAsyncEventArgs()) {
                    args.SetBuffer(buffer, index, count);
                    client.Client.SendAsync(args);
                }
            }
            catch (Exception ex){
                CloseClientSocket(client.Client);
            }
        }
