...
public class BlueprintGUILayout:Editor
{
    ...
    static object DrawPropertyField(object value, System.Type type, int selectTypeIndex = -1, params GUILayoutOption[] options)
    {
        if (value == null) { EditorGUILayout.TextField("null--value!!!", options); return value; }
        switch (type.FullName) {
            case "System.Int32":
        ...
        if (SystemType.IsSubclassOf(type, typeof(System.Enum)))
            return EditorGUILayout.EnumPopup((System.Enum)value, options);
        if (type.FullName == "System.Type") {
    ...
    static object DrawPropertyField(string label, object value, System.Type type, int selectTypeIndex = -1, params GUILayoutOption[] options)
    {
        if (value == null) { EditorGUILayout.TextField("null--value!!!", options); return value; }
        switch (type.FullName) {
            case "System.Int32":
        ...
        if (SystemType.IsSubclassOf(type, typeof(System.Enum))) {
            if (!value.GetType().IsEnum) value = CreateUtility.CreateInstance(type);
            return EditorGUILayout.EnumPopup(label, (System.Enum)value, options);
        }
        if (type.FullName == "System.Type") {
