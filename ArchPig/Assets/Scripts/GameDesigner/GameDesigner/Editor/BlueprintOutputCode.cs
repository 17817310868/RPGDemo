...
namespace GameDesigner
{
	public class BlueprintOutputCode : EditorWindow
	{
		...
		static public BlueprintOutputCode instance
		{
			...
			{ 
				if( window == null )
					window = EditorWindow.GetWindow <BlueprintOutputCode> ( "蓝图代码生成编辑器" , true );
				return window;
			}
			set{ window = value; }
		}

		[MenuItem("StateMachine/BlueprintCodeGenerationEditor")]
		static void init()
		{
			window = instance;
		}
			...
			foreach( var m in designer.methods ){
				...
				if( m.method.typeModel == ObjectModel.Object ){
					...
					default:
						...
						}else if( m.method.targetType.GetConstructors().Length == 0 ){//没有构造函数,声明的变量不能赋值
							text+="\tpublic " + m.method.targetType.Name + " " + m.method.nodeName + " = null;\n";
		...
		private string IFPars( Parameter par )
		{
			...
			if( par.setValue ){//如果给参数赋值
				...
				if( metype == MemberTypes.Method | metype == MemberTypes.Property ){
					if( par.parameterTypeName != "System.Object" & par.parameterTypeName != par.setValue.method.returnTypeName ){//如果类型不同,就要强制转换类型才能编译通过
					...
					if( par.parameterTypeName != "System.Object" & par.parameterTypeName != par.setValue.method.returnTypeName ){//如果变量类型与节点返回类型不同,就要强制转换类型才能编译通过
				...
				}else if( metype == MemberTypes.Custom | metype == MemberTypes.Constructor ){//如果节点是对象,比较对象类型
					if( par.parameterTypeName != "System.Object" & par.parameterTypeName != par.setValue.method.targetTypeName ){// (这里使用的对象类型) 如果类型不同,就要强制转换类型才能编译通过
						str += "(" + par.parameterType.Name + ")" + NInvoke( par.setValue ); // 设置参数值
						if(!namespaces.Contains("using "+par.parameterType.Namespace+";")&par.parameterType.Namespace!=null&par.parameterType.Namespace!="System"){
							namespaces.Add ("using "+par.parameterType.Namespace+";");
						}
		...
		private string SetLoaclPar(Parameter par)
		{
			...
			if( SystemType.IsSubclassOf( par.parameterType , typeof(UnityEngine.Object) ) ){
				...
				default:
					...
					}else if( par.parameterType.GetConstructors().Length == 0 ){//如果参数类型没有构造函数,赋值一个空的值,不能创建对象
						str += "null";
					}else{
		...
		{
			string text = "";
			switch( body.method.memberTypes ){//第一个被赋值的都是主对象 target = gameobject.transform = other.transform;
			case MemberTypes.Method://如果第一个节点属于方法
				if( body.method.name == "IF" & Contains(body)){
					...
					if( body.setValue ){//如果给对象赋值
						if( body.method.methodInfo.IsStatic )//如果方法是静态方法使用类名点方法名调用,不需要声明对象
							Debug.Log( "静态方法不能设置对象值,直接使用类名点方法调用!" );
						else{//当给对象赋值,为了考虑赋值的长度问题,用一个()框起来,对象赋值完成后再点方法名
							if( body.method.name.Contains("set_") ){//如果方法名包含set_,说明这是属性被方法获得到的,当编译后无法使用,所以不能加()
					...
					}else{//当没有给对象赋值,使用自动对象调用方法
						if( body.method.methodInfo.IsStatic ){//如果方法是静态方法使用类名点方法名调用,不需要声明对象
							if( body.method.name.Contains("get_") ){//如果方法名包含get_,说明这是属性被方法获得到的,当编译后无法使用,所以不能加()
			...
			case MemberTypes.Property:
				if( body.setValue ){//如果给对象赋值
					if( body.method.propertyInfo.CanWrite ){//如果属性可写才能赋值
					...
					if( body.method.propertyInfo.CanWrite ){//如果属性可写才能设置属性值
						if( body.method.propertyInfo.GetSetMethod().IsStatic ){//如果方法是静态方法使用类名点方法名调用,不需要声明对象
			...
			case MemberTypes.Field:
				if( body.setValue ){//如果给对象赋值
					...
					if( body.method.fieldInfo.IsStatic ){//如果方法是静态方法使用类名点方法名调用,不需要声明对象
						text += tab + body.method.targetType.Name + "." + body.method.name + " = ";//设置参数值
						if(!namespaces.Contains("using "+body.method.targetType.Namespace+";")&body.method.targetType.Namespace!=null&body.method.targetType.Namespace!="System"){
							namespaces.Add ("using "+body.method.targetType.Namespace+";");
						}
		...
		public string NInvoke( BlueprintNode body , MemberTypes onEnter = MemberTypes.Event )
		{
			...
			case MemberTypes.Property://当属性在次方法时都属于获得值(获得属性值) , 所以不允许在获得属性值的同时设置属性值 , 要想设置属性值请在第一个节点(主方法)才能设置
				if( body.setValue ){//如果给对象赋值
					if( body.method.propertyInfo.CanRead ){//如果属性可读才能获得属性值
					...
					if( body.method.propertyInfo.CanRead ){//如果属性可读才能获得属性值
						if( body.method.propertyInfo.GetGetMethod().IsStatic ){//如果方法是静态方法使用类名点方法名调用,不需要声明对象
							str += body.method.targetType.Name + "." + body.method.name;//设置参数值
							if(!namespaces.Contains("using "+body.method.targetType.Namespace+";")&body.method.targetType.Namespace!=null&body.method.targetType.Namespace!="System"){
								namespaces.Add ("using "+body.method.targetType.Namespace+";");
							}
			...
			case MemberTypes.Field:
				if( body.setValue ){//如果给对象赋值
					if (body.method.fieldInfo.IsStatic)//如果变量是静态不能给对象赋值
						Debug.Log ("静态变量不能设置对象值,直接使用类名点方法调用!");
					else if(body.method.valueModel == Method.ValueModel.Get) {//如果字段属于获得类型，那么就不需要给字段赋值
						str += "(" + NInvoke( body.setValue ) + ")." + body.method.name;//设置参数值
					}else{//当给对象赋值,为了考虑赋值的长度问题,用一个()框起来,对象赋值完成后再点方法名
					...
					if( body.method.fieldInfo.IsStatic ){//如果方法是静态方法使用类名点方法名调用,不需要声明对象
						if(body.method.valueModel == Method.ValueModel.Get) {//如果字段属于获得类型，那么就不需要给字段赋值
							str += body.method.targetType.Name + "." + body.method.name;//设置参数值
						}else
							str += body.method.targetType.Name + "." + body.method.name + " = ";//设置参数值
						if(!namespaces.Contains("using "+body.method.targetType.Namespace+";")&body.method.targetType.Namespace!=null&body.method.targetType.Namespace!="System"){
